...
export async function GET(request: NextRequest) {
  ...
  if (resolved !== "true") {
    ...
    // Build a set of already-present anomalies for dedup. CRUCIAL: include
    // ALL persisted anomalies in the date range, not just the ones currently
    // filtered into `result`. Otherwise, when the user hides resolved
    // anomalies (default), a computed anomaly that has been dismissed
    // (resolved=true in DB) gets re-generated by the live calculation and
    // reappears immediately — making the "Corretto" button look broken.
    const persistedAll = await prisma.anomaly.findMany({
      where: from && to ? { date: { gte: from, lte: to } } : {},
      select: { employeeId: true, date: true, type: true, description: true },
    });
    const existingSet = new Set<string>();
    for (const a of result)
      existingSet.add(`${a.employeeId}|${a.date}|${a.type}|${a.description}`);
    for (const a of persistedAll)
      existingSet.add(`${a.employeeId}|${a.date}|${a.type}|${a.description}`);
