...
/**
 * Parser delle date per il comando /ferie.
 *
 * Formati accettati (case insensitive):
 *   DAL 15/04 AL 18/04
 *   DAL 15/04/2026 AL 18/04/2026
 *   15/04 - 18/04
 *   15/04
 *
...
export function parseLeaveDates(input: string): { startDate: string; endDate: string } | null {
  ...
  // Pattern 2: "gg/mm[/aaaa] - gg/mm[/aaaa]" o "gg/mm[/aaaa] AL gg/mm[/aaaa]"
...
function buildDate(dd: string, mm: string, yyyy?: string): string | null {
  ...
  // Costruisci e valida che il giorno esista nel mese (es. 31/02 fallisce)
  const d = new Date(year, month - 1, day);
  if (d.getFullYear() !== year || d.getMonth() !== month - 1 || d.getDate() !== day) return null;
  return `${year}-${String(month).padStart(2, "0")}-${String(day).padStart(2, "0")}`;
}

function formatItDate(yyyymmdd: string): string {
